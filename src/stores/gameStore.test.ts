...
describe("initial state", () => {
  ...
  it("starts with 0 oil, empty stations and contracts", () => {
...
describe("tick", () => {
  ...
  it("advances and removes expired contracts", () => {
  ...
  it("decrements contract timeRemaining", () => {
...
describe("buyContract", () => {
  ...
  it("2nd contract costs 110 currency", () => {
    reset({ currency: 1_000, contracts: [makeContract()] });
    useGameStore.getState().buyContract();
    expect(useGameStore.getState().currency).toBe(890);
  });

  it("3rd contract costs 120 currency", () => {
    reset({ currency: 1_000, contracts: [makeContract({ id: 1 }), makeContract({ id: 2 })] });
    useGameStore.getState().buyContract();
